...
int main(int argc, char **argv) {
  ...
  {
    std::size_t pos;
    infiles_per_job = std::stoi(arg, &pos);
    if (pos < arg.size()) {
      std::cerr << "Trailing characters after number: " << arg << std::endl;
    }
  } catch (const std::invalid_argument& e) {
  ...
  for (size_t j = 0; i < num_jobs; j++) {
    auto task = workflow.addTask("task_"+j, average_flops, 1, 1, average_memory);
    for (size_t f = 0; f < infiles_per_job; f++) {
      task->addInputFile(workflow.addFile("infile_"+j+"_"+f, average_infile_size));
    }
    task->addOutputFile(workflow.addFile("outfile_"+j, 0.0));
  }
  ...
  for (std::vector<std::string>::iterator hostname = hostname_list.begin(); hostname != hostname_list.end(); ++hostname)
  {
    ...
    if (*hostname != wms_host) {
      ...
      if (
        ...
        auto remote_storage_service = simulation.add(new wrench::SimpleStorageService(storage_host, {"/"}));
        remote_storage_services.insert(remote_storage_service);
      } else {
  ...
  auto htcondor_compute_service = simulation.add(
    new wrench::HTCondorComputeService(
      wms_host,
      std::move(condor_compute_resources),
      {
  ...
  simulation_output = simulation.getOutput();
  ...
  std::cerr << "Dumping generated data..."
